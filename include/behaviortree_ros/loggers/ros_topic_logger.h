...
public:
      ...
      NodeStatus status) override
  {
    ...
    ros_status.value = toROS(status);
    event.status = ros_status;
    ros_status.value = toROS(prev_status);
  ...
  /**
   * @brief Write the current BT to the ROS message to publish
   * @param tree
   */
  void writeBT(const BT::Tree &tree){
    event_log_.behavior_tree = BTtoROS(tree);
  }
